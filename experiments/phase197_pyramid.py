"""
Phase 197: Spatial Pyramid Memory - Position-Preserving Global Context

P196 failed: 1x1 pooling kills position info.
Solution: Pool to 2x2 and 4x4, then upsample back. Preserves
spatial topology ("red blob in top-right") without O(N^2) attention.
...
class SpatialPyramidNCA(nn.Module):
    """NCA with spatial pyramid pooling for position-preserving global context."""
    def __init__(self, n_colors=11, hidden_ch=64, embed_dim=32, n_steps=5):
        ...
        # Pyramid projectors: compress channels before pooling
        pyr_ch = C // 4  # lightweight
        self.pyr_2x2 = nn.Conv2d(C, pyr_ch, 1)
        self.pyr_4x4 = nn.Conv2d(C, pyr_ch, 1)

        # Update: local state + pyramid context
        self.update = nn.Sequential(
            nn.Conv2d(C + pyr_ch * 2, C, 3, padding=1), nn.ReLU(),
            nn.Conv2d(C, C, 1),
        )
    ...
    def forward(self, x, task_emb):
        ...
        for t in range(self.n_steps):
            # Spatial Pyramid: pool to 2x2 and 4x4, then upsample back
            ctx_2x2 = F.adaptive_avg_pool2d(self.pyr_2x2(state), (2, 2))
            ctx_2x2 = F.interpolate(ctx_2x2, size=(H, W), mode='nearest')

            ctx_4x4 = F.adaptive_avg_pool2d(self.pyr_4x4(state), (4, 4))
            ctx_4x4 = F.interpolate(ctx_4x4, size=(H, W), mode='nearest')

            # Combine local + pyramid context
            combined = torch.cat([state, ctx_2x2, ctx_4x4], dim=1)
...
def main():
    ...
    t0 = time.time()
    print("=" * 70)
    print("Phase 197: Spatial Pyramid Memory")
    print(f"  Position-preserving global context (2x2 + 4x4 pyramid)")
    ...
    print(f"\n[Spatial Pyramid NCA]")
    m2 = SpatialPyramidNCA(11, C, 32, 5).to(DEVICE)
    print(f"  Params: {m2.count_params():,}")
    h2 = train_and_eval(m2, train, test, ep, "Pyramid")
    del m2; gc.collect(); torch.cuda.empty_cache() if DEVICE == "cuda" else None

    elapsed = time.time() - t0
    d = (h2[-1] - h1[-1]) * 100
    print(f"\n{'='*70}")
    print(f"Phase 197 Complete ({elapsed:.0f}s)")
    print(f"  Standard: {h1[-1]*100:.1f}%  |  Pyramid: {h2[-1]*100:.1f}%  |  Delta: {d:+.1f}pp")
    print(f"{'='*70}")

    os.makedirs(RESULTS_DIR, exist_ok=True)
    with open(os.path.join(RESULTS_DIR, "phase197_pyramid.json"), 'w', encoding='utf-8') as f:
        json.dump({'standard': h1[-1], 'pyramid': h2[-1], 'elapsed': elapsed,
    ...
    try:
        ...
        ax.plot(epochs, [h*100 for h in h1], 'o-', color='#95a5a6', lw=2, label='Standard')
        ax.plot(epochs, [h*100 for h in h2], 'D-', color='#e67e22', lw=2, label='Spatial Pyramid')
        ax.set_xlabel('Epoch'); ax.set_ylabel('Test PA (%)'); ax.legend(); ax.grid(True, alpha=0.3)
        ax.set_title(f'Phase 197: Spatial Pyramid (Δ={d:+.1f}pp)', fontweight='bold')
        os.makedirs(FIGURES_DIR, exist_ok=True)
        fig.savefig(os.path.join(FIGURES_DIR, 'phase197_pyramid.png'), dpi=150, bbox_inches='tight')
        plt.close(); print("  Figure saved!")
    except Exception as e:
        print(f"  Figure error: {e}")
    gc.collect()
    return {'standard': h1[-1], 'pyramid': h2[-1]}
